...
export default function Card({ recipe }) {
  ...
  return (
    <div className="recipe-card" id={recipe.calories}>
      <img src={recipe.image} alt={recipe.label} />
      <h2>{recipe.label}</h2>
      <p>{recipe.cuisineType[0].charAt(0).toUpperCase() + recipe.cuisineType[0].slice(1)}</p>
      <a href={recipe.shareAs} className="button">
        To recipe
      </a>
    </div>
  );
